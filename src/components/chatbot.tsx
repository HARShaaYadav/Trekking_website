...
const GREETING: ChatMessage = {
    from: "bot",
    text: "Namaste 🙏 I'm TrekBot, the AI assistant for Trekking Arunachal Pradesh. I can recommend a trek by region, difficulty or duration, and answer questions about prices and the best seasons. Try one of the quick questions below, or just type away!",
};
...
function getBotReply(raw: string): string {
    ...
    if (
        ...
        const names = easy
            ...
            .join("\n");
        return `Great news — Arunachal Pradesh has some wonderfully gentle routes for your first trek. My top picks for beginners:\n${names}\n\nThe Sangestar Tso (Madhuri Lake) Loop is usually our most recommended starting point. Want more detail on any?`;
    }
    ...
    if (text.includes("difficult") || text.includes("challenging") || text.includes("hard") || text.includes("experienced") || text.includes("adventure")) {
        ...
        const names = hard
            ...
            .map((t) => `• ${t.name} — ${t.days} days, ${t.grade}`)
            .join("\n");
        return `If you're after a real challenge, these are our most demanding routes:\n${names}\n\nFor the ultimate experience, the Gorichen Base Camp or Bailey Trail treks are unforgettable.`;
    }
