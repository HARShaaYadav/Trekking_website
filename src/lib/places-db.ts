...
export async function listPublicTreks(): Promise<Trek[]> {
    try {
        ...
        // This site has a deliberately curated Arunachal Pradesh catalogue.
        // Retain database edits to those routes, but do not surface legacy
        // Arunachal Pradesh rows that may still exist from an earlier version of the site.
        for (const t of dbTreks) {
...
export async function getPublicTrek(slug: string): Promise<Trek | null> {
    try {
        ...
        if (dbTrek && baseTreks.some((t) => t.slug === dbTrek.slug)) {
            return dbTrek;
        }
    } catch (err) {
        console.error("[treks] DB unavailable, falling back to seed data:", err);
